...
// No "Services" entry: /pest-control 404s on non-pest tenants and no
// vertical-neutral services route exists — the nav Services dropdown covers it.
const QUICK_LINKS = [
  { label: 'Home', href: '/' },
    ...
    <footer style={{ backgroundColor: 'var(--color-footer-bg)', color: 'var(--color-footer-text)' }}>
      ...
      <div className="bg-black/10 py-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row justify-between items-center gap-2 text-sm text-gray-500">
          ...
          <span className="text-xs" style={{ color: 'var(--color-primary)' }}>Powered by HomeFlow Pro</span>
        </div>
      </div>
      <div className="bg-black/20 py-2.5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="text-xs text-gray-600">Website by <span className="font-semibold" style={{ color: 'var(--color-primary)' }}>HomeFlow Pro</span> · A product of Ironwood Operations Group</span>
        </div>
      </div>
    </footer>
  );
