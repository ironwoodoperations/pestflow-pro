...
const colTitle: React.CSSProperties = {
  ...
  color: 'var(--vg-gold-light)', marginBottom: '1rem',
};
const colLink: React.CSSProperties = {
  display: 'block', padding: '0.35rem 0', textDecoration: 'none',
  fontFamily: 'var(--vg-font-body)', fontWeight: 300, fontSize: 14, color: 'var(--vg-cream)',
};
...
export function VitaGlowFooter({ tenant, social = {}, bookingUrl }: Props) {
  ...
  return (
    <footer style={{ background: 'var(--vg-ink)', color: 'var(--vg-cream)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '4rem 1.5rem 2rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: '2.5rem' }}>
        <div style={{ minWidth: 200 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
            <VitaGlowGlyph size={30} tone="cream" />
            <span style={{ fontFamily: 'var(--vg-font-display)', fontWeight: 400, fontSize: 24, letterSpacing: 'var(--vg-tracking-display)', color: 'var(--vg-cream)' }}>{bizName}</span>
          </div>
          {tenant.tagline && (
            <p style={{ marginTop: '1rem', fontFamily: 'var(--vg-font-display)', fontStyle: 'italic', fontWeight: 300, fontSize: 18, lineHeight: 'var(--vg-line-body)', color: 'rgba(250,247,242,0.75)', maxWidth: 260 }}>{tenant.tagline}</p>
          )}
          {phone && <a href={`tel:${tel}`} style={{ ...colLink, marginTop: '1rem', color: 'var(--vg-gold-light)' }}>{formatPhone(phone)}</a>}
          {tenant.address && <p style={{ ...colLink, color: 'rgba(250,247,242,0.7)' }}>{tenant.address}</p>}
        </div>
      ...
      <div style={{ borderTop: '1px solid rgba(201,162,39,0.25)', maxWidth: 1200, margin: '0 auto', padding: '1.25rem 1.5rem', display: 'flex', flexWrap: 'wrap', gap: '0.75rem 1.5rem', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem 1.5rem' }}>
          {LEGAL.map((l) => <Link key={l.href} href={l.href} style={{ ...colLink, padding: 0, fontSize: 12.5, color: 'rgba(250,247,242,0.65)' }}>{l.label}</Link>)}
        </div>
        <p style={{ fontFamily: 'var(--vg-font-body)', fontWeight: 300, fontSize: 12.5, color: 'rgba(250,247,242,0.55)' }}>© 2026 {bizName}</p>
      </div>

      <div style={{ textAlign: 'center', padding: '1rem', borderTop: '1px solid rgba(201,162,39,0.25)' }}>
