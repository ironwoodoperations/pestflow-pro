    ...
    <footer style={{ backgroundColor: '#ffffff', borderTop: '1px solid #e5e7eb', color: '#374151' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            {tenant.logo_url
              ? <img src={tenant.logo_url} alt={name} className="h-10 w-auto object-contain mb-3"
                  onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
              : <p className="text-lg font-bold mb-3" style={{ color: '#1a1a1a' }}>{name}</p>
            }
            {tenant.tagline && <p className="text-sm text-gray-500 mb-2">{tenant.tagline}</p>}
            {tenant.address && <p className="text-sm text-gray-500 mb-1">{tenant.address}</p>}
            {phone && (
              <a href={`tel:${phone.replace(/\D/g, '')}`} className="text-sm font-semibold block mb-2 transition"
          ...
          <div>
            ...
            <ul className="space-y-2">
              {SERVICE_LINKS.map((l) => (
                <li key={l.href}>
                  <Link href={l.href} className="text-sm text-gray-500 hover:text-gray-800 transition">{l.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            {phone && (
              <p className="text-sm mb-1">
                <a href={`tel:${phone.replace(/\D/g, '')}`} className="font-semibold transition"
                  style={{ color: 'var(--color-primary)' }}>{formatPhone(phone)}</a>
              </p>
            )}
            {tenant.email && (
              <p className="text-sm">
                <a href={`mailto:${tenant.email}`} className="text-gray-500 hover:text-gray-800 transition">{tenant.email}</a>
              </p>
            )}
          </div>
        </div>
      ...
      <div style={{ borderTop: '1px solid #e5e7eb' }} className="py-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row justify-between items-center gap-3 text-xs text-gray-400">
          ...
          <div className="flex items-center gap-3">
            ...
            <span>·</span>
            <Link href="/sms-terms" className="hover:text-gray-600 transition">SMS Terms</Link>
          </div>
          <a href="https://pestflowpro.com" target="_blank" rel="noopener noreferrer"
            className="transition hover:opacity-80" style={{ color: 'var(--color-primary)' }}>
