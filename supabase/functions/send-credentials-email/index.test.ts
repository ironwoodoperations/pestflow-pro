...
//   deployed function (verifies behavior in the actual Supabase Edge
//   Runtime, not just in Deno locally).
//
// Run:
//   deno test --allow-env --allow-net supabase/functions/send-credentials-email/

import { assertEquals } from 'https://deno.land/std@0.224.0/assert/mod.ts'

// Module-scoped consts in index.ts read these at import time. Set BEFORE the
// dynamic import below. Use a non-resolvable URL so any auth.getUser fetch
// fails fast on DNS rather than hanging.
...
function installFetchSpy(): { calls: FetchCall[]; restore: () => void } {
  ...
  // Default-OK responder: auth.getUser → null user (rejects), Resend → 200 ok.
  // Cases 1, 2, 5 should never reach Resend (gate rejects first or body parse
  // fails first); the spy just confirms zero Resend hits.
  globalThis.fetch = ((input: string | URL | Request, init?: RequestInit) => {
    const url = typeof input === 'string' ? input : input instanceof URL ? input.toString() : input.url
    calls.push({ url, init })
    if (url.includes('/auth/v1/user')) {
      // supabase-js auth.getUser → returns null user when token invalid
...
function makeReq(opts: { authHeader?: string; body?: unknown }): Request {
  ...
  return new Request('http://local.test/send-credentials-email', {
    method: 'POST',
    headers,
...
const validBody = {
  ...
  siteUrl: 'https://example-pest.pestflowpro.com',
  adminUrl: 'https://example-pest.pestflowpro.com/admin',
  adminEmail: 'admin@example-pest.com',
  adminPassword: 'TempPass2026!',
}

Deno.test('1. Authorization header missing → 401, no Resend call', async () => {
...
Deno.test('2. Invalid bearer token → 403, no Resend call', async () => {
  // auth.getUser returns null user → handler treats as Forbidden (since
  // !user matches the second clause of the gate condition).
  const spy = installFetchSpy()
  try {
...
Deno.test('5. Body validation runs AFTER gate (case 3/4 covered by post-deploy curl)', async () => {
  // This case verifies the gate ordering: even with an invalid token + invalid
  // body, the response is the gate rejection (403), NOT a 400 body-error.
  // Confirms gate runs BEFORE req.json() body parse — which is the security
