...
// constraint. The prompt now (1) declares the platform (see PLATFORM_NAME) as the only one the
...
<style>
  ...
  .snapshot{margin:14px 0 0;padding:10px 12px;background:#f8fafc;border:1px solid #e5e7eb;border-radius:8px;color:#6b7280;font-size:13px;line-height:1.45}
  .snapshot strong{color:#4b5563;font-weight:600}
...
<body><div class="wrap">
  <header>
    ...
    <p class="snapshot">This is a snapshot of your website's health as of ${escapeHtml(periodLabel)}. As you apply fixes in your <strong>SEO → Pages</strong> tab, your live to-do list updates there — this report stays fixed at what we found this month. Always check the <strong>SEO → Pages</strong> tab for what's left to do.</p>
  </header>
  ${sections.join('\n  ')}
  <footer>Generated by ${escapeHtml(PLATFORM_NAME)} · ${escapeHtml(periodLabel)}</footer>
</div></body></html>`
}
