...
// the post-commit sync drain in offboard-tenant didn't finish. Internal-secret
// authed; pg_cron POSTs every 5 min. Exponential backoff via attempts + updated_at
// (no extra column). Advances audit final_state when a request's rows are terminal.
//
// DEPLOY verify_jwt:false. Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY,
//   ZERNIO_API_KEY, PROCESS_OFFBOARD_QUEUE_INTERNAL_SECRET.
...
const INTERNAL_SECRET = Deno.env.get('PROCESS_OFFBOARD_QUEUE_INTERNAL_SECRET') || ''
...
serve(async (req) => {
  ...
  if (!INTERNAL_SECRET) { console.error('[process-offboard-queue] secret not set'); return json(500, { error: 'Server misconfigured' }) }
  if (!constantTimeEq(INTERNAL_SECRET, req.headers.get('apikey') || '')) {
  ...
  try {
    ...
    const summary = { scanned: rows?.length ?? 0, processed: 0, deferred: 0, done: 0, skipped_404: 0, pending: 0 }
    ...
    for (const row of (rows ?? []) as QueueRow[]) {
      ...
      else if (r.status === 'skipped_404') summary.skipped_404++
      else summary.pending++
      touched.add(row.request_id)
    }

    // advance audit for any request whose rows are now all terminal
