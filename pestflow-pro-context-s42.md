# PestFlow Pro — Project Context (Session 42 Ready)
...
- Do NOT generate a session 43 context file. End with a plain summary only.
...
## BUILD STATUS (end of S41)

- Build: 0 TS errors
- Bundle: 384 kB — under 450 kB limit ✅
- Subdomain routing: live
- All shells: stable — do NOT modify in S42
- Tenant auth isolation: complete and verified
- provision-tenant v4: fully hardened and self-contained
  - Auto-creates tenants row from slug (idempotent)
  - Admin API user creation (no NULL columns)
  - Inserts into tenant_users
  - Seeds all 11 settings keys
  - Returns tenant_id in response
- Client Setup Wizard: delivery-ready
  - Correct pricing everywhere ($99/$149/$249/$499)
  - Template selector (4 shells)
  - Success screen with live URL + credentials + copy buttons
- Dashboard plan card: upgraded comparison dropdown with mailto upgrade CTA
- Firecrawl migration tool: scripts/migrate-site.ts — working
- Platform is ready to deliver a real client in under 10 minutes
...
## PROVISION-TENANT (v4) — FULLY HARDENED

1. Auto-creates tenants row from slug — no manual Supabase step needed
2. Creates auth user via Admin API — no NULL column issues
3. Inserts into tenant_users — has_role passes on first login
4. Seeds all 11 settings keys with correct shapes
5. Returns tenant_id in response — success screen uses it
...
| provision-tenant | ACTIVE v4 | Self-contained, no manual DB steps |

---

## SHELL SYSTEM (stable — do not modify in S42)
...
| 41 | Apr 2026 | provision-tenant v4. Wizard delivery-ready. Prices fixed. Template picker. Success screen. Plan card upgrade dropdown |
...
| Session | Focus |
|---------|-------|
| S42 | First real paying client delivery — this session |
| S43+ | Second client + iteration based on real-world feedback |
...
### Goal
Deliver the first real paying client. Run the wizard live on a call or async,
hand them credentials, confirm their site is live.

This session is primarily operational — Scott drives the wizard, Claude Code
stands by to fix anything that breaks in real conditions.

---

### Pre-Session Checklist (Scott to confirm before starting)

- [ ] Client company name, phone, email, address confirmed
- [ ] Client slug chosen and confirmed unique
- [ ] Client plan tier agreed (Starter/Grow/Pro/Elite)
- [ ] Client shell preference chosen (or Scott picks for them)
- [ ] 1Password vault has all platform credentials
- [ ] pestflowpro.com/admin accessible and logged in as demo admin

---

### Focus 1 — Deliver First Client

1. Open Client Setup Wizard at pestflowpro.com/admin → Client Setup
2. Fill in all client details
3. Select plan and shell
4. Review step — confirm slug, email, password, live URL
5. Click "Provision Client & Download Setup Doc"
6. Copy credentials from success screen
7. Confirm [slug].pestflowpro.com is live and loading client data
8. Confirm [slug].pestflowpro.com/admin/login works with generated credentials
9. Hand credentials to client (email or 1Password share)
10. Send onboarding email if not auto-fired

---

### Focus 2 — Fix Anything That Breaks

If anything fails during Focus 1, fix it in this priority order:
1. Provisioning error — fix provision-tenant edge function
2. Login loop — check tenant_users, settings keys, has_role
3. Public site not loading — check subdomainRouter, tenant slug
4. Wrong data showing — check settings seed values

For each fix: commit and push immediately, then re-test.

---

### Focus 3 — Post-Delivery Polish (if time allows)

Based on what you see during the real delivery, note anything that felt clunky
or missing. Fix up to 3 small UX issues. Do not add new features.

---

## SESSION 42 TASK ORDER

1. Confirm pre-session checklist complete
2. Run Client Setup Wizard for real client
3. Verify provisioning success
4. Test client login and public site
5. Hand off credentials to client
6. Fix anything broken — commit after each fix
7. Note UX issues observed during delivery — fix up to 3
8. Final build — 0 TS errors, bundle under 450 kB
9. Plain summary only — no context file

---

## SESSION 42 STARTER BLOCK
...
Session 41 is complete. provision-tenant is v4 and fully self-contained. The Client
Setup Wizard is delivery-ready — correct pricing, template picker, success screen with
copy buttons. Bundle: 384 kB.

Session 42 is the first real client delivery. Scott will run the wizard live.
Stand by to fix anything that breaks in real conditions. Fix and commit immediately
after each issue — do not batch fixes.
