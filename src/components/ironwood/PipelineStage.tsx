...
export default function PipelineStage({ prospectId, stage, onChanged }: Props) {
    ...
    <div>
          ...
          return (
            <div key={s.id} className="flex items-center">
              <button
                ...
                title={isGated ? 'QA gate required — complete in Reveal Queue' : undefined}
                className={`px-2.5 py-1 rounded text-xs font-semibold whitespace-nowrap transition ${
                  isCurrent ? 'bg-blue-600 text-white' :
                  isDone    ? 'bg-green-500 text-white' :
                  isGated   ? 'bg-gray-200 text-gray-400 cursor-not-allowed opacity-60' :
      ...
      {gateMsg && (
        <p className="text-xs text-amber-400 mt-1.5">
          QA gate required — complete in Reveal Queue (Session C)
        </p>
      )}
