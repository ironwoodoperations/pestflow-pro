...
export default function LocationsTab() {
  ...
  async function fetchServiceAreas() {
    if (!tenantId) return
    const { data } = await supabase.from('service_areas').select('*').eq('tenant_id', tenantId).order('city')
  ...
  useEffect(() => {
    if (!tenantId) return
    supabase.from('service_areas').select('*').eq('tenant_id', tenantId).order('city')
