  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '14px 18px', background: 'rgba(0,0,0,0.3)', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontFamily: F.h, fontWeight: 700, fontSize: 13, color: C.white }}>Lead Inbox</span>
        <span style={{ background: 'rgba(6,182,212,0.15)', border: '1px solid rgba(6,182,212,0.3)', borderRadius: 20, padding: '2px 10px', fontSize: 10, color: C.teal, fontFamily: F.b, fontWeight: 600 }}>3 New</span>
      </div>
      <div style={{ padding: '8px 0' }}>
        {leads.map((l, i) => (
          <div key={l.name} style={{
            ...
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, #1a3a5c, #06B6D4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: C.white, fontFamily: F.h, flexShrink: 0 }}>
              {l.name.charAt(0)}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: F.b, fontWeight: 600, fontSize: 13, color: C.white, marginBottom: 2 }}>{l.name}</div>
              <div style={{ fontFamily: F.b, fontSize: 11, color: C.muted }}>{l.service} · {l.time}</div>
            </div>
            <div style={{
              background: l.status === 'New' ? 'rgba(6,182,212,0.15)' : l.status === 'Contacted' ? 'rgba(16,185,129,0.12)' : 'rgba(251,191,36,0.12)',
              border: `1px solid ${l.status === 'New' ? 'rgba(6,182,212,0.3)' : l.status === 'Contacted' ? 'rgba(16,185,129,0.3)' : 'rgba(251,191,36,0.3)'}`,
              borderRadius: 20, padding: '2px 10px', fontSize: 10,
              color: l.status === 'New' ? C.teal : l.status === 'Contacted' ? C.green : '#fbbf24',
      ...
      <div style={{ padding: '12px 18px', background: 'rgba(0,0,0,0.2)', borderTop: '1px solid rgba(255,255,255,0.05)', textAlign: 'center' }}>
        <a href="https://pestflow-pro.pestflowpro.com/admin" target="_blank" rel="noopener noreferrer" style={{ fontFamily: F.b, fontSize: 12, fontWeight: 600, color: C.teal, textDecoration: 'none' }}>
          See the dashboard →
        </a>
      </div>
    </div>
    ...
    <section style={{ background: C.bg, padding: '96px 32px' }}>
      <div style={{ maxWidth: 1160, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 64 }}>
          <div style={{ fontFamily: F.b, fontSize: 12, fontWeight: 700, color: C.teal, letterSpacing: 2, marginBottom: 12, textTransform: 'uppercase' }}>Lead Management</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, marginBottom: 64 }}>
          {MINI.map(m => (
            <div key={m.title} style={{ background: '#0d1526', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '28px 26px', textAlign: 'center' }}>
              <div style={{ fontSize: 32, marginBottom: 14 }}>{m.icon}</div>
              <h3 style={{ fontFamily: F.h, fontWeight: 700, fontSize: 16, color: C.white, margin: '0 0 10px' }}>{m.title}</h3>
