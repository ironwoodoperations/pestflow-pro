...
export default function About() {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--color-bg-section)' }}>
      ...
      <section className="py-12" style={{ backgroundColor: 'var(--color-bg-cta)' }}>
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="font-oswald tracking-wide text-3xl md:text-4xl mb-6" style={{ color: 'var(--color-nav-text)' }}>Our Mission</h2>
          <p className="text-lg leading-relaxed" style={{ color: 'var(--color-nav-text)', opacity: 0.85 }}>To protect Texas homes and businesses with science-backed pest control solutions delivered by local professionals who treat your property like their own.</p>
        </div>
      </section>

      <section className="py-12" style={{ background: 'linear-gradient(135deg, var(--color-bg-hero) 0%, var(--color-bg-hero-end) 100%)' }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[
              ...
              <div key={s.label}>
                ...
                <div className="text-sm mt-1" style={{ color: 'var(--color-nav-text)', opacity: 0.6 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {VALUES.map((v, i) => {
              ...
              return (
                <div key={v.title} className="rounded-xl p-6 flex gap-4" style={{ backgroundColor: 'var(--color-primary-light)' }}>
                  <div className="flex-shrink-0 mt-1" style={{ color: 'var(--color-primary)' }}><Icon className="w-7 h-7" /></div>
                  <div>
                    <h3 className="text-lg font-bold mb-1" style={{ color: 'var(--color-heading)' }}>{v.title}</h3>
      ...
      <section className="py-16" style={{ backgroundColor: 'var(--color-bg-cta)' }}>
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="font-oswald tracking-wide text-3xl md:text-4xl text-center mb-10" style={{ color: 'var(--color-nav-text)' }}>Meet Our Team</h2>
          {team === null ? null : team.length === 0 ? (
            <p className="text-center text-base" style={{ color: 'var(--color-nav-text)', opacity: 0.6 }}>Our team will be featured here.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {team.map((member) => (
                <div key={member.id} className="rounded-xl p-6 text-center shadow-sm" style={{ backgroundColor: 'var(--color-bg-section)' }}>
                  ...
                  <h3 className="text-lg font-bold" style={{ color: 'var(--color-heading)' }}>{member.name}</h3>
      ...
      <section className="py-16" style={{ backgroundColor: 'var(--color-bg-section)' }}>
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="font-oswald tracking-wide text-3xl md:text-4xl text-center mb-10" style={{ color: 'var(--color-heading)' }}>Licensed & Certified</h2>
          <div className="flex flex-wrap gap-4 justify-center">
            {['NPMA Member', 'TPCA Certified', 'BBB Accredited', 'TDA Licensed', 'EPA Certified', 'WDI Inspector'].map((cert) => (
              <div key={cert} className="rounded-xl px-6 py-4 text-center font-medium border border-gray-200 shadow-sm text-sm" style={{ backgroundColor: 'var(--color-primary-light)', color: 'var(--color-heading)' }}>{cert}</div>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-16" style={{ backgroundColor: 'var(--color-bg-cta)' }}>
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-lg mb-8" style={{ color: 'var(--color-nav-text)', opacity: 0.75 }}>Get your free quote today — same-day service available.</p>
