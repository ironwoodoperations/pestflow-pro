...
export default function ContactPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (ownerSmsNumber) {
      fetch(fnUrl, { method: 'POST', headers, body: JSON.stringify({ tenant_id: tenantId, to: ownerSmsNumber, message: `📬 New contact from ${form.name} — ${form.phone} — "${form.message?.slice(0, 80)}". Check CRM: https://pestflow-pro.vercel.app/admin`, type: 'owner' }) }).catch(() => {})
    }
