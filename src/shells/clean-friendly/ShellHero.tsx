...
export default function ShellHero() {
  ...
  useEffect(() => {
    ...
    setHomeContent(c)
    if (c.subtitle) setHeroSubtext(c.subtitle)
  ...
  return (
    <>
      <section
        ...
        style={{ backgroundImage: `url(${bgImage})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', position: 'relative' }}
      >
        {bgImage && <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 0, pointerEvents: 'none' }} />}
        <div className="relative z-10 text-center px-4 py-16">
          {biz.name && <p className="text-xl font-semibold tracking-widest uppercase mb-2" style={{ color: 'rgba(255,255,255,0.9)' }}>{biz.name}</p>}
          {biz.phone ? (
          ...
          <Link to="/quote" style={{ backgroundColor: 'var(--color-btn-bg)', color: 'var(--color-btn-text)' }}
      ...
      <div className="bg-gray-900">
        <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-gray-700">
          {STRIPS.map((s, i) => (
            <Link key={i} to={s.href} className="flex flex-col items-start py-6 px-8 cursor-pointer hover:bg-gray-800 transition group">
