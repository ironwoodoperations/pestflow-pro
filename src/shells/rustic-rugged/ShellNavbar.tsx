      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="hidden lg:flex items-center gap-5 flex-1 justify-center">
            <div ref={dropRef} className="relative" onMouseEnter={() => { if (timer.current) clearTimeout(timer.current); setDropOpen(true) }} onMouseLeave={() => { timer.current = setTimeout(() => setDropOpen(false), 150) }}>
              ...
              {dropOpen && (
                <div role="menu" className="absolute top-full left-0 mt-1 w-52 bg-white shadow-xl rounded-lg border border-gray-100 py-2 z-50">
                  {SERVICE_LINKS.map(l => (
                    <Link key={l.href} to={l.href} onClick={() => setDropOpen(false)}
            ...
            {NAV_LINKS.map(l => (
              <Link key={l.href} to={l.href} className="text-sm font-medium transition" style={{ color: '#1a1a1a' }}
                onMouseEnter={e => (e.currentTarget.style.color = 'var(--color-primary)')}
                onMouseLeave={e => (e.currentTarget.style.color = '#1a1a1a')}>
                {l.label}
              </Link>
          ...
          <div className="hidden lg:flex items-center gap-3 shrink-0">
            ...
            <Link to="/quote" className="font-bold rounded px-5 py-2.5 text-sm text-white transition hover:opacity-90" style={{ backgroundColor: 'var(--color-primary)' }}>Free Estimate</Link>
          </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 max-h-[80vh] overflow-y-auto">
          <div className="px-4 py-4 space-y-1">
            ...
            {SERVICE_LINKS.map(l => <Link key={l.href} to={l.href} onClick={() => setMobileOpen(false)} className="block px-2 py-2 text-sm font-medium" style={{ color: '#1a1a1a' }}>{l.label}</Link>)}
            <div className="border-t border-gray-100 my-2" />
            {NAV_LINKS.map(l => <Link key={l.href} to={l.href} onClick={() => setMobileOpen(false)} className="block px-2 py-2 text-base font-medium" style={{ color: '#1a1a1a' }}>{l.label}</Link>)}
            ...
            <Link to="/quote" onClick={() => setMobileOpen(false)} className="block text-center font-bold rounded px-5 py-2.5 text-white transition mt-3" style={{ backgroundColor: 'var(--color-primary)' }}>Free Estimate</Link>
          </div>
        </div>
      )}
    </nav>
