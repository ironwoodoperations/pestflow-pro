...
export default function ServicesSection() {
  return (
    <section className="py-16" style={{ background: 'hsl(45,95%,60%)', backgroundImage: 'url(https://www.dangpestcontrol.com/wp-content/uploads/2025/03/background-pest-control.png)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundBlendMode: 'multiply' }}>
      <div className="container mx-auto px-4">
        <p className="dang-text-comic text-sm text-center mb-2 italic" style={{ color: 'hsl(28,100%,50%)' }}>OUR SERVICES</p>
        <h2 className="dang-text-comic text-4xl md:text-5xl text-center mb-3 uppercase" style={{ color: 'hsl(20,40%,12%)' }}>Our Pest Control Services</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {services.map((s) => (
            <Link key={s.slug} to={`/${s.slug}`} className="flex flex-col items-center gap-3 text-center group">
              <img src={s.img} alt={s.name} className="w-24 h-24 object-cover rounded-xl group-hover:scale-105 transition-transform duration-200" />
              <h3 className="dang-text-comic text-sm uppercase" style={{ color: 'hsl(20,40%,12%)' }}>{s.name}</h3>
            </Link>
          ))}
        </div>
        <div className="text-center mt-10">
          <Link to="/quote" className="inline-flex items-center justify-center font-bold rounded-full px-10 py-3 text-white text-base transition-all"
