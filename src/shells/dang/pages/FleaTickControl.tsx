    ...
    <div style={{ fontFamily: "'Open Sans', sans-serif", color: 'hsl(20, 40%, 12%)', overflowX: 'hidden' }}>
      ...
      <section className="px-4 md:px-10" style={{ paddingTop: '80px', paddingBottom: '60px', maxWidth: '1200px', margin: '0 auto', background: '#ffffff', backgroundImage: 'radial-gradient(circle, #d0d0d0 1px, transparent 1px)', backgroundSize: '24px 24px' }}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center">
          ...
          <div>
            <p style={{ fontFamily: '"Bangers", cursive', color: 'hsl(28, 100%, 50%)', fontSize: '18px', letterSpacing: '0.12em', fontStyle: 'italic', marginBottom: '6px', marginTop: 0 }}>{content?.subtitle ?? 'FLEAS & TICKS'}</p>
            <h2 style={{ fontSize: 'clamp(26px, 2.8vw, 38px)', fontWeight: '800', marginBottom: '18px', marginTop: 0 }}>{content?.title ?? 'Flea & Tick Control Service'}</h2>
            <p style={{ fontSize: '16px', lineHeight: 1.75, marginBottom: '28px', color: '#444', marginTop: 0 }}>{content?.intro ?? 'Fleas and ticks are more than just an annoyance—they\'re a threat to your family\'s comfort and enjoyment outdoors. At{\' \'} Dang Pest Control, we provide professional flea and tick pest control services that effectively eliminate these pests and help you maintain a healthy, itch-free environment. Our customized treatments will eliminate any existing fleas and ticks and will also prevent future intrusions. We serve Tyler, TX, and the surrounding areas. Call us today at{\' \'} (903) 871-0550{\' \'}and{\' \'} get your quote.'}</p>
      ...
      <section className="px-4 md:px-10" style={{ background: '#f1f1ef', paddingTop: '70px', paddingBottom: '70px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <h2 style={{ textAlign: 'center', fontWeight: '800', fontSize: 'clamp(26px, 3vw, 40px)', marginBottom: '10px', marginTop: 0 }}>Comprehensive Flea & Tick Control Process</h2>
          <p style={{ textAlign: 'center', color: '#666', marginBottom: '50px', marginTop: 0 }}>At Dang Pest Control, our technicians take a thorough, integrated approach to handle flea and tick infestations. Here's how we ensure your family stays protected:</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center">
          ...
          <div style={{ position: 'relative', padding: '20px' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1, border: '4px solid rgb(255, 213, 39)', borderRadius: '6px', overflow: 'hidden', boxShadow: '8px 8px 0 rgba(0,0,0,0.1)' }}>
              <img loading="lazy" width={600} height={400} src="https://www.dangpestcontrol.com/wp-content/uploads/2025/05/Pets-Marvel-Outside-scaled-e1746459229132.jpg" alt="Pet and Dog Flea Control Services in Tyler TX" style={{ width: '100%', display: 'block' }} />
            </div>
          </div>
        </div>
      </section>

      <style>{`.ft-cta-phone { background: hsl(28,100%,50%) !important; color: #fff !important; border: 2px solid hsl(28,100%,50%) !important; } .ft-cta-quote { background: #fff !important; color: hsl(28,100%,50%) !important; border: 2px solid #fff !important; }`}</style>
