...
const CATEGORIES = ['Residential', 'Commercial', 'Termite Control', 'Mosquito Control', 'Rodent Control']
...
export default function ShellHomeSections() {
  ...
  return (
    <>
      ...
      <section className="py-8 bg-[#1c1c1c] overflow-x-auto">
        <div className="flex gap-3 px-6 min-w-max">
          {CATEGORIES.map((cat) => (
            <a
              key={cat}
              href="/services"
              className="px-6 py-2 rounded-full border border-amber-500 text-amber-400 font-semibold text-sm hover:bg-amber-500 hover:text-black transition whitespace-nowrap"
            >
              {cat}
            </a>
          ))}
