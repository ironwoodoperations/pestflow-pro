...
export default function MetroProFaqTabs({ faqs: dbFaqs }: Props) {
  const [activeTab, setActiveTab] = useState('general')

  const currentTab = TABS.find(t => t.id === activeTab) || TABS[0]
  const faqData = currentTab.faqs
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl md:text-4xl font-bold" style={{ color: 'var(--color-heading)' }}>
            Get Answers to Common Pest Control Questions
          </h2>
        </div>
            ...
            <button
              ...
              style={
                activeTab === tab.id
                  ...
                  : { backgroundColor: '#f1f3f5', color: '#555' }
              }
            >
              {tab.label}
            </button>
        ...
        <div className="border border-gray-100 rounded-xl px-6 bg-gray-50">
          <Accordion faqs={dbFaqs?.length ? dbFaqs.map(f => ({ q: f.question, a: f.answer })) : faqData} />
        </div>
      </div>
    </section>
  )
