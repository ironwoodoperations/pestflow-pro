...
export default function MetroProReviews({ testimonials }: Props) {
  const reviews = testimonials?.length ? testimonials.slice(0, 3) : STATIC_REVIEWS
          ...
          <div className="inline-flex items-center gap-3 mb-3">
            ...
            <div>
              <div className="flex gap-0.5 mb-1">
                {[1,2,3,4,5].map(i => <Star key={i} className={`w-5 h-5 ${i <= 5 ? 'text-amber-400 fill-amber-400' : ''}`} />)}
              </div>
