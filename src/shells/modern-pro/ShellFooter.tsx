    ...
    <footer style={{ backgroundColor: 'var(--color-footer-bg)', color: 'var(--color-footer-text)' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {QUICK_LINKS.map((link) => <li key={link.href + link.label}><Link to={link.href} className="hover:text-emerald-400 transition">{link.label}</Link></li>)}
            </ul>
          </div>
          <div>
      ...
      <div className="bg-[#040810] py-2.5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="text-xs text-gray-600">Website by <span className="text-[#10b981] font-semibold">PestFlow Pro</span> · A product of Ironwood Operations Group</span>
        </div>
      </div>
    </footer>
  )
