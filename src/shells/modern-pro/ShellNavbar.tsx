...
export default function ShellNavbar() {
  ...
  return (
    <nav className="bg-[#0a0f1e] border-b border-white/10 sticky top-0 z-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link to="/" className="flex items-center gap-3 font-oswald text-2xl tracking-wide">
            ...
            <div>
              <span className="text-emerald-400 font-bold">{businessName.split(' ')[0]}</span>{' '}
          ...
          <div className="hidden lg:flex items-center gap-5">
            <div ref={dropdownRef} className="relative" onMouseEnter={handleDropdownEnter} onMouseLeave={handleDropdownLeave}>
              <button aria-haspopup="true" aria-expanded={dropdownOpen} className="text-sm font-medium text-gray-300 hover:text-emerald-400 transition flex items-center gap-1">
                ...
                <div role="menu" className="absolute top-full left-0 mt-1 w-56 bg-[#141922] shadow-xl rounded-lg border border-white/10 py-2 z-50">
                  {SERVICE_LINKS.map((link) => (
                    <Link key={link.href} to={link.href} onClick={() => setDropdownOpen(false)}
                      className="block px-4 py-2 text-sm text-gray-300 hover:text-emerald-400 hover:bg-white/5 transition">
                      {link.label}
                    </Link>
                  ))}
                </div>
            ...
            {NAV_LINKS.map((link) => (
              <Link key={link.href} to={link.href} className="text-sm font-medium text-gray-300 hover:text-emerald-400 transition">{link.label}</Link>
            ))}
            <Link to="/quote" className="bg-emerald-500 hover:bg-emerald-400 text-white font-bold rounded-lg px-5 py-2.5 transition">Get Free Quote</Link>
          </div>
      ...
      {mobileOpen && (
        <div ref={menuRef} className="lg:hidden bg-[#0a0f1e] border-t border-white/10 max-h-[80vh] overflow-y-auto">
          <div className="px-4 py-4 space-y-1">
            <p className="text-xs font-semibold text-gray-500 uppercase px-2 pt-2 pb-1">Services</p>
            {SERVICE_LINKS.map((link) => (
              <Link key={link.href} to={link.href} onClick={() => setMobileOpen(false)} className="block px-2 py-2 text-sm text-gray-300 hover:text-emerald-400 transition">{link.label}</Link>
            ))}
            <div className="border-t border-white/10 my-2" />
            {NAV_LINKS.map((link) => (
              <Link key={link.href} to={link.href} onClick={() => setMobileOpen(false)} className="block px-2 py-2 text-base font-medium text-gray-300 hover:text-emerald-400 transition">{link.label}</Link>
            ))}
            <Link to="/quote" onClick={() => setMobileOpen(false)} className="block text-center bg-emerald-500 hover:bg-emerald-400 text-white font-bold rounded-lg px-5 py-2.5 transition mt-3">Get Free Quote</Link>
          </div>
        </div>
      )}
    </nav>
