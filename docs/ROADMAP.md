...
- **The apex host-to-tenant mapping is duplicated and the two copies disagree (from S313's gate; both models say fix BOTH sides, server-side authoritative).** `subdomainRouter.ts:39` maps `pestflowpro.ai` and `www.pestflowpro.ai` to the master tenant; `slugFromRequest` in `password-reset-request` returns `null` for the apex and `'www'` for `www`. So the client renders a **"Forgot password?" button that cannot work** — `App.tsx:78` routes `/admin/login` unconditionally, `Login.tsx:159` has no host gate, the user is told "check your email", and no email is sent. Three dead surfaces, by two different paths: the apex → `no_slug`, `www.pestflowpro.ai` → `tenant_not_found` (slug resolves to `www`), custom domains → `no_slug`.

  **The fix shape, from Perplexity and worth keeping verbatim:** ONE canonical host-to-tenant mapping shared by the router and the function, with **contract tests over apex, `www`, custom domains, unknown hosts, absent Origin, and Referer fallback**. Duplicating the mapping is what let the two drift; a shared module plus that test matrix is what stops it recurring. S313 reports this and deliberately does not fix it.

### S320 — from the validator gate (recorded, deliberately NOT in the S320 PR)

All three are real, both models raised them, and none belongs inside an RLS migration.
Recorded here so they are not re-derived from scratch next time.

- **Canonical object-key validation, client-side (Perplexity NON-BLOCKING; Gemini (f) item 1).** The storage policies compare `(storage.foldername(name))[1]` as **text**, so a malformed first path segment is a clean non-match and denies the row — the RLS side is safe and deliberately so. What is missing is the write-side guard: the upload API should **require a canonical UUID first segment** and reject top-level objects, empty first segments, traversal-shaped strings, and unexpectedly normalized Unicode. Gemini adds the specific case: an object written to a bucket root (`logos/header.png` rather than `logos/<tenant_id>/header.png`) yields `NULL` from `foldername[1]`, which correctly *denies* — meaning a legitimate file uploaded to the wrong path becomes **permanently unreachable through the authenticated API** rather than erroring at upload time. Fix at the client, not in RLS.

- **Audit logging for operator-bypass writes (Perplexity NON-BLOCKING).** `is_operator()` on the `logos` bucket is a cross-tenant mutation capability held by one named person. It is the right design and it is now revocable, but **nothing records its use**. Log actor id, target tenant prefix, bucket, object key, operation and a request correlation id whenever a write lands via the operator branch rather than the membership branch.

- **Are `tenant-assets` and `videos` correctly PUBLIC buckets? (Perplexity NON-BLOCKING; Gemini (c) concurs on the mechanism.)** Four of five buckets are `public = true`, so Supabase serves `/object/public/…` **without evaluating RLS** and the SELECT policies gate only the authenticated path and `list`. That is fine for logos. It is a question worth answering for `tenant-assets` (customer property photos) and `videos`. Public/private is **bucket-wide** — a tenant folder cannot be made private inside a public bucket — so the answer is either "leave public" or "make the bucket private and move delivery to signed URLs", with no middle option.

- **Cross-tenant copy/move is unbounded by these policies (Gemini (f) item 2).** Supabase copy/move needs `SELECT` on the source and `INSERT` on the destination. Nothing builds this today; if tenant asset duplication is ever added, the source and destination prefixes must be evaluated **separately** or a copy becomes a cross-tenant write.
